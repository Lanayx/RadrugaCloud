namespace Infrastructure.InfrastructureTools
{
    ...
    public class MemberNameResolver : IMemberNameResolver
    {
        ...
        private static IEnumerable<MemberInfo> GetMembers(Type type)
        {
            if (type.IsInterface)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach (var subInterface in subType.GetInterfaces().Where(x => !considered.Contains(x)))
                    {
                    ...
                    var typeProperties =
                        subType.GetMembers(BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.Instance);
        ...
        private static bool HasAliasAttribute(string alias, MemberInfo member)
        {
            ...
            var xmlElement = attributes.OfType<XmlElementAttribute>().FirstOrDefault();
        ...
        {
            var member = GetMembers(type).Select(
                x =>
                    {
        ...
        private static string ResolveNameInternal(MemberInfo member)
        {
            ...
            var xmlElement =
                member.GetCustomAttributes(typeof(XmlElementAttribute), true)
            ...
            var xmlAttribute =
                member.GetCustomAttributes(typeof(XmlAttributeAttribute), true)
